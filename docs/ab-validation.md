...
- Далі потрібен додатковий крок агрегації на боці LLM/клієнта

**Відповідь, яку видав:**
- Сервер повернув лише вміст файлу (без готового списку SKU і без total)

**Правильно?**
Частково. Дані правильні, але кінцева бізнес-відповідь не сформована автоматично без додаткової логіки.
...
| Загальна сума 46152.00 точна | Так | Ні (без окремої агрегації) |
| Скільки кроків знадобилось | 2 (`low_stock` + `resources/read`) | 1 читання файлу + додаткова обробка поза MCP |
...
Custom MCP server дає якісну різницю в shape відповіді: одразу повертає бізнес-сутність (SKU на дозамовлення + inventory summary), а не сирі дані. Без `catalog-server` залишається тільки читання JSON, після чого потрібна додаткова агрегація на боці моделі/клієнта. Для цього кейсу MCP-сервер виправданий тим, що перетворює "доступ до файлу" на стабільний доменний API.
